...
--31)Select words whose first character is 'h', last character is 't' and second character is from 'a' to 'e'
SELECT word
FROM words
WHERE REGEXP_LIKE(word, 'h[a-e]t');--[a-e] means a,b,c,d,e

--32)Select words whose first character is 's' or 'a' or 'y'
SELECT word
FROM words
WHERE REGEXP_LIKE(word, '^[say](*)');--(*) is optional to use

--33)Select words whose last character is 'm' or 'a' or 'f'
SELECT word
FROM words
WHERE REGEXP_LIKE(word, '(*)[mfa]$');--(*) is optional to use

--34)Select words whose first character is 's' and last character is 'a'
SELECT word
FROM words
WHERE REGEXP_LIKE(word, '^[s].*[a]$');--.* use for multiple or zero characters between first and last characters

--35)Select words which have 'a' in any position
SELECT word
FROM words
WHERE REGEXP_LIKE(word, 'a');--'a' in REGEXP_LIKE same with %a% in LIKE

--36)Select words which have characters from 'd' to 't' at the beginning followed by any character then 'l'
SELECT word
FROM words
WHERE REGEXP_LIKE(word, '^[d-t].[l]');-- . in REGEXP_LIKE same with _ in LIKE

--37)Select words which have characters from 'd' to 't' at the beginning followed by any 2 characters then 'e'
SELECT word
FROM words
...
--40)Select words which do not end with 't' and do not end 'f'
SELECT word
FROM words
WHERE word NOT LIKE '%t' AND word NOT LIKE '%f';--be careful about using AND, it is because of the logic rules

--41)Select words which start with any character, not followed by 'a', not followed by 'e'
SELECT word
FROM words
...
SELECT *
FROM words
ORDER BY number_of_letters ASC;--ASC is optional, it is ascending as default

--42) Put the records in descending order by using word
SELECT *
FROM words
...
SELECT *
FROM words
ORDER BY 3 ASC;--3 stands for number_of_letters field

SELECT *
FROM words
ORDER BY 2 DESC;--2 stands for word field

--42) Put the records in descending order by using name field, and ascending order by using points field
CREATE TABLE points
(
    name VARCHAR2(50),
    point NUMBER(3)
